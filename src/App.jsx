...
const App = () => {
  ...
  const lineChartData = {
    labels: Array.from({ length: projectionYears * 12 }, (_, i) => `Month ${i + 1}`),
    datasets: participatingEntities.map((entity, index) => ({
      label: entity.name,
      data: Array.from({ length: projectionYears * 12 }, () => (entity.percentage / 100) * (creditsGenerated * (historicalData.length > 0 ? historicalData[historicalData.length - 1].price : 0))),
      borderColor: `hsl(${index * 360 / participatingEntities.length}, 70%, 50%)`,
      backgroundColor: `hsl(${index * 360 / participatingEntities.length}, 70%, 50%)`,
      pointRadius: 4,
      pointHoverRadius: 6,
      fill: false,
      tension: 0.1
  ...
  const lineChartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `${projectionYears} Year Monthly Revenue Projection`,
      },
    },
    scales: {
      x: {
